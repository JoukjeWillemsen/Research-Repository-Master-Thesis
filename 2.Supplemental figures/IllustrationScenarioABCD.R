...
VOORBEELD=ggplot(data = dataVOORBEELD, aes(x=x,y=y, group=group)) +
  ...
  annotate("text", x = 0, y = dnorm(lambda_cA)+0.08, label = expression(lambda*"c"), color="#56B4E9") +
  
  geom_segment(aes(x = lambda_eA, y = 0, xend = lambda_eA, yend = -dnorm(lambda_eA, mean = lambda_eA, sd=1)), color="#009E73")+ #exp
  
  annotate("text", x = 0, y = - dnorm(lambda_eA) - 0.08, label = expression(lambda*"e"), color="#009E73") +
...
B=ggplot(dataB, aes(x=x,y=y, group=group)) +
  ...
  scale_color_manual(values = c("#999999", "#56B4E9", "#009E73"), name = "") +#rood #009E73 blauw #009E73 blauw
...
library(ggplot2)
ggsave("4.Test.png", plot = annotatedfigure, width = 7, height = 5)
